...
int network_send(struct buffer &b, uint32_t missed, size_t *total)
{
	...
	if (debug)
		LOGI("STARTING BATCH WITH %u SAMPLES!\n", b.num_samples);
	...
	for (index = 0; index < b.num_samples; ++index) {
		...
		if (packet_should_create(b, s, pi)) {
			if (debug)
				LOGI("PACKET MUST BE SENT BEFORE CONTINUING.\n");
			if (transmit(&sent))
				return -1;
			sent_total += sent;
		}
...
int network_init(const char *node, const char *service)
{
	...
	if (connect(the_socket, addr->ai_addr, addr->ai_addrlen)) {
		...
		LOGI("connect error:  %s\n", strerror(errno));
		return errno;
	}

	if (debug) {
		LOGI("NETWORK INITIALIZED!\n");
...
int network_packet(void *packet, size_t bytes, size_t *sent)
{
	...
	if (debug)
		LOGI("Writing out packet with %zu bytes.\n", bytes);
	...
	if (sent_bytes != bytes) {
		LOGI("Error, could not send entire packet.\n");
		return -1;
	}
	if (network_debug && !fwrite(packet, 1, bytes, network_debug)) {
		LOGI("Error writing out network debug info:  %s\n", strerror(errno));
		return -1;
	}

	if (debug)
		LOGI("Packet successfully sent.\n");
